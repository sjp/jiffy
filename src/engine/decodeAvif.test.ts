...
const u32 = (n: number): Uint8Array => { const a = new Uint8Array(4); new DataView(a.buffer).setUint32(0, n, false); return a; };
...
const avisCompat = box('ftyp', enc.encode('avif'), u32(0), enc.encode('avif'), enc.encode('avis'), enc.encode('miaf'));
...
await assert.rejects(() => decodeAvif(ab(avisMajor)), /ImageDecoder/, 'throws when ImageDecoder unavailable');
...
class FakeVideoFrame {
  ...
  constructor(durationUs: number | null) { this.duration = durationUs; }
  close() {}
}
const decodedIndexes: number[] = [];
let closed = false;
class FakeImageDecoder {
  tracks = { ready: Promise.resolve(), selectedTrack: { frameCount: 3, animated: true } };
