...
function parseAnimatedWebP(buf: ArrayBuffer): {
  ...
  if (frames.length === 0)
    throw new Error('decodeWebP: no ANMF frames found');
...
function makeFrameBlob(frameData: ArrayBuffer, width: number, height: number): Blob {
  ...
  return new Blob([hdr, vp8x, new Uint8Array(frameData)], { type: 'image/webp' });
}
...
export async function decodeWebP(bytes: ArrayBuffer): Promise<DecodeResult> {
  const { canvasWidth, canvasHeight, bgRGBA, frames: rawFrames } = parseAnimatedWebP(bytes);
